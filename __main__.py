...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("Updates Channel", "https://t.me/+eumGiaaLyAFlZDU1")
    buttons.buildbutton("Owner", "https://t.me/Irfan50786")
    uname = f'<a href="tg://user?id={update.message.from_user.id}">{update.message.from_user.first_name}</a>'
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''Hey! {uname},\n
This bot can mirror all your links to Google Drive! and also can Upload them on Telegram if you want. You can use me to Download any direct link & any Youtube-Dl Supported link to Telegram. Any Problem DM:-@Irfan50786
